...
w = Tk()
w.geometry('300x200')
w.title('Instagram users Scrapper!!')
...
def scrape():
    usr = input('Whose Followers You Need To Scrap (Enter UserName)__:  ')

    user_input = int(
        input('How many Followers Do You Wanna Scrap (eg. 500) '))
    ...
    WebDriverWait(bot, TIMEOUT).until(
        EC.presence_of_element_located((
            By.XPATH, '//*[@id="react-root"]/div/div/section/main/div/ul/li[2]/a/span'))).click()
    ...
    print('Saving...')
    print('Your followers are saved in followers.txt file!')
...
label = Label(w, text='Please use And\nFollow the instructions in\nconsole Window', bg='black', fg='white')
label.pack(pady=20)
button = Button(w, text='Scrap users', bg='green', fg='white', command=start_it)
button.pack(pady=20)
